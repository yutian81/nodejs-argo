...
async function extractDomains() {
  ...
  async function generateLinks(argoDomain) {
      let ISP;
      try {
          // 步骤 1: 使用 axios 替代 curl，异步获取 Cloudflare 的 meta 信息
          const response = await axios.get('https://speed.cloudflare.com/meta');
          const metaData = response.data; // axios 会自动将返回的 JSON 字符串解析为对象
  
          // 步骤 2: 在 JavaScript 中直接从对象中提取所需信息，替代 awk 和 sed
          const colo = metaData.colo || 'N/A'; // Cloudflare 数据中心位置
          const organization = metaData.asOrganization || 'N/A'; // ISP/组织信息
          
          // 步骤 3: 格式化字符串，并将所有空格替换为下划线
          ISP = `${colo}-${organization}`.replace(/ /g, '_');
  
      } catch (error) {
          // 如果网络请求失败，打印错误信息并提供一个默认值
          console.error('Failed to fetch Cloudflare meta information:', error.message);
          ISP = 'Appwrite-Platform'; 
      }
  
      // 等待一个固定的时间，确保其他进程（如 bot）有时间完成初始化
      await new Promise(resolve => setTimeout(resolve, 2000));
